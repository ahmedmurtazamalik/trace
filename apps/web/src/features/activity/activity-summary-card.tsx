...
interface Props { item: ActivitySummary; headingLevel?: 2 | 3; index?: number }

export function ActivitySummaryCard({ item, headingLevel = 2, index }: Props) {
  ...
  return <Card className="activity-event-card" style={index === undefined ? undefined : { "--row-index": index } as React.CSSProperties}>
    ...
    <time dateTime={item.occurredAt}>{formatPakistanDateTime(item.occurredAt)}</time>
  </Card>;
}
