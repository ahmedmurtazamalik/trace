...
export function ActivityExperience({ loadActivity, loadRepositories, initialFilters = {}, fixedFilters = {}, timezone = "UTC", onFiltersChange }: ActivityExperienceProps) {
  ...
  return <div className="activity-experience">
    ...
    {loading && items.length === 0 ? <Card className="activity-state-card" role="status">Loading development activity…</Card>
      ...
      : items.length === 0 ? <Card className="activity-state-card"><h2>{Object.keys(filters).length ? "No activity matches these filters" : "No development activity yet"}</h2><p>{Object.keys(filters).length ? "Clear filters or choose a broader combination." : "If tracking is already enabled, verify that GitHub App webhook deliveries can reach Trace, then push a new commit. Trace records new events after tracking starts and does not backfill older commits."}</p>{Object.keys(filters).length > 0 ? <Button className="trace-button-secondary" onClick={clear}>Clear filters</Button> : <Link className="trace-button trace-button-primary" href="/repositories">Review repository tracking</Link>}</Card>
      : <section className="activity-timeline" aria-label="Development activity timeline">{Object.entries(groupByDay(items, timezone)).map(([date, group]) => <section className="activity-day-group" key={date}><h2>{new Date(`${date}T12:00:00.000Z`).toLocaleDateString("en-US", { dateStyle: "long", timeZone: "UTC" })}</h2><ul>{group?.map((item) => <li key={item.id}><ActivitySummaryCard headingLevel={3} item={item} /></li>)}</ul></section>)}</section>}
