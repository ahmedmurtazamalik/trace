...
export class WorkspaceAnalysisCollector {
  ...
  async collect(input: WorkspaceAnalysisRepositoryInput, fromSha: string | null): Promise<WorkspaceAnalysisCollection> {
    const token = await this.installationToken(input.githubInstallationId);
    const toSha = await this.head(token, input);
    ...
    const changes = fromSha === null || fromSha === toSha ? [] : await this.compare(token, input, fromSha, toSha);
    ...
    const evidence = workspaceAnalysisEvidenceSnapshotSchema.parse({
      ...
      baselineOnly: fromSha === null,
      files,
      changes,
      exclusions,
    });
    return { toSha, dataCutoffAt: new Date(), coverage, evidence };
  }
  ...
  private async compare(token: string, input: WorkspaceAnalysisRepositoryInput, fromSha: string, toSha: string): Promise<WorkspaceAnalysisEvidenceSnapshot['changes']> {
    const value = await this.api(`/repos/${encodeURIComponent(input.owner)}/${encodeURIComponent(input.name)}/compare/${fromSha}...${toSha}`, `Bearer ${token}`, {}, 5 * 1024 * 1024) as { files?: unknown };
