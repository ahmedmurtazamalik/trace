...
- Forgot-password responses remain identical for known and unknown identifiers. Eligible-account issuance runs asynchronously behind a renewable per-user Redis lock, while the public response completes in the same randomized minimum window even if delivery is slower. Each issuer snapshots prior token IDs under a PostgreSQL user-row lock and can retire only that snapshot, so an older issuer cannot consume a newer token even if Redis ownership changes.
