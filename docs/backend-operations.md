...
Automatic Slack delivery runs only after the report and current PDF artifact commit successfully. The finalization transaction records a pending render-generation audit event with the bounded notification snapshot, making report completion and its delivery obligation durable together. Later audit records serialize delivery claims, record outcomes, and suppress posts whenever a success exists for that exact revision and generation. A same-revision regeneration increments the generation and receives a new notification. Definitive failures, ambiguous attempted-only outcomes, and accepted posts whose success audit could not be persisted use the report job's bounded retries without reverting the report's completed status. Before posting, every retry authenticates the unresolved audit target against the durable report/revision relationship, scope, report date, and stored executive summary; malformed or mismatched obligations fail closed before mutable generation work. Slack incoming webhooks do not accept an idempotency key, so a worker crash after Slack accepts a message but before Trace records success can cause a duplicate on retry; the boundary intentionally favors eventual delivery over silently dropping a finalized report.
