...
The report protocol is frozen in `packages/shared/src/reports.ts`; validated list/detail fixtures live under `packages/shared/test/fixtures/reports/`. Day 8 implements authenticated create/list/detail handlers, deterministic factual aggregation, immutable input snapshots, and durable queue publication. AI, revision persistence, LaTeX, artifact storage, and download streaming remain Days 9–10.
