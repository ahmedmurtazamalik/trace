...
`apps/worker/src/queues/github/github-webhook.worker.ts` and `apps/worker/src/runtime.ts` provide the Day 5 worker lifecycle boundary with Redis readiness, concurrency restricted to 1–32, bounded durable-reference validation, idempotent deadline-bounded close, SIGINT/SIGTERM handling, and awaited terminal-failure recording. BullMQ stores only the stable `WEBHOOK_PROCESSING_FAILED` reason; raw exception text is never retained. The executable fails closed until Day 6 supplies a real processor, so Day 5 cannot consume accepted jobs as no-ops and does not enrich commits, call AI, or generate reports.
