...
`apps/worker/src/queues/github/github-webhook.worker.ts`, `apps/worker/src/processors/github/github-push.processor.ts`, and `apps/worker/src/runtime.ts` provide the implemented webhook worker lifecycle: Redis readiness, concurrency restricted to 1–32, bounded durable-reference validation, execution-time authority revalidation, canonical PostgreSQL activity persistence, monitored fatal run-loop completion, SIGINT/SIGTERM handling, and graceful-then-forced deadline-bounded close. Terminal-failure recording is attempted once and recorder failures are contained; BullMQ stores only the stable `WEBHOOK_PROCESSING_FAILED` reason and never retains raw processor or observability exception text. A fatal run loop marks the process failed and initiates the same idempotent stop path.
