...
| --- | --- | --- | --- | --- | --- |
| `POST` | `/api/v1/github/connect` | `200` | authenticated session + `X-CSRF-Token`; no body | `GithubConnectResponse` containing a backend-generated HTTPS `github.com` authorization URL; the same operation is used when reconnecting | `401 UNAUTHENTICATED`; `403 CSRF_INVALID`; `429 RATE_LIMITED` |
...
| `POST` | `/api/v1/github/installation` | `200` | authenticated connected session + `X-CSRF-Token`; no body | `GithubInstallationStartResponse` containing a state-bound `github.com/apps/.../installations/new` URL | `401 UNAUTHENTICATED`; `403 CSRF_INVALID`; `409 GITHUB_RECONNECT_REQUIRED`; `429 RATE_LIMITED` |
...
| Method | Path | Success | Request | Success response | Documented errors |
| --- | --- | --- | --- | --- | --- |
| `POST` | `/api/v1/repositories/sync` | `200` | authenticated session + `X-CSRF-Token`; no body | `{ "accessibleRepositoryCount": number }` | `401 UNAUTHENTICATED`; `403 CSRF_INVALID`; `409 GITHUB_INSTALLATION_REQUIRED`; `409 GITHUB_INSTALLATION_SUSPENDED`; `429 RATE_LIMITED`; `503 SERVICE_UNAVAILABLE` |
...
Dashboard queries require an ISO date, accept an IANA timezone (default `UTC`), and optionally filter by opaque repository ID. Metrics and the latest 20 summaries include only canonical GitHub `commit`, `push`, and `pull_request` events from the caller's currently tracked, currently accessible repositories, bounded to the requested half-open local calendar day. File, addition, and deletion totals come only from validated canonical commit metadata to avoid double counting push summaries. An unknown, stale, foreign, or inaccessible repository ID returns indistinguishable `404 REPOSITORY_NOT_FOUND`; the browser treats it as a permanent filter state and offers to clear the repository filter without revealing existence. After connection and tracking preconditions, state precedence is: `PARTIAL` while any authorized delivery for the selected local day and optional repository is pending or processing, even before activity materializes; otherwise `NO_ACTIVITY` when the canonical aggregate is empty; otherwise `READY`. Deliveries for inaccessible, untracked, removed, suspended, disconnected, out-of-day, foreign-user, or repository-filter-mismatched repositories do not affect state.
...
The shared contract reserves sources `github | cli` and their closed activity types. The Day 7 API currently returns only GitHub `commit | push | pull_request` rows. CLI rows fail closed until the opt-in local CLI persists explicit Trace-user/device ownership; repository membership alone is not sufficient authority for local work. Each returned item contains repository context, an optional contributor who need not have a Trace account, source/type, timestamp, and strict nullable factual display fields (`sha`, message, branch, files changed, additions, deletions, and optional evidence URL). Provider credentials, webhook internals, raw patches, and arbitrary metadata are excluded.
...
The endpoint establishes a bounded request correlation ID before body parsing, then reads at most 256 KiB into a raw buffer. It verifies the HMAC-SHA256 signature over those exact bytes with a timing-safe comparison before decoding JSON. The signed push envelope then validates bounded ref, exactly 40- or 64-character hexadecimal object IDs, installation, stable repository ID, repository full name, sender, and every nested commit field (IDs, messages, timestamps, URLs, authors, and added/removed/modified path arrays). Impossible intermediate object-ID lengths, missing/malformed headers, or invalid schema return `400`; invalid signatures return `401`; oversized bodies return `413 WEBHOOK_PAYLOAD_TOO_LARGE` with the same request ID. General API JSON remains limited to 1 MiB. URL-encoded parsing is deliberately disabled; authenticated mutations use JSON plus CSRF protection.
...
The API publishes one BullMQ job named `process-github-webhook`, using deterministic ID `github-webhook-<delivery-row-id>`, five bounded exponential-backoff attempts, and `{ "deliveryId": "<delivery-row-id>" }` as its entire Redis payload. PostgreSQL is the durable source of both the validated payload and the queue-publication obligation: every `pending` row remains owed regardless of its prior `publishedAt` marker, so a lost Redis job is recreated with the same deterministic ID. The request performs a bounded best-effort publish, while an independent startup/periodic reconciler rotates null/oldest attempts fairly. Each pass locks the delivery and exact authority rows and commits attempt clocks for the selected cohort before publication begins. Publication then reacquires and revalidates those locks around a one-second queue wait inside an expiring transaction; the oldest obligation is tried first and the rest of the cohort cannot be serialized indefinitely behind one hung queue call. A rejecting or hanging poison row therefore rotates behind later pending work, and a crash between phases remains recoverable. Revocation observed before the bounded queue command terminally fails the delivery; because a timed-out remote Redis command can have an ambiguous outcome, worker execution revalidates authority and terminally rejects any already or ambiguously queued job before canonical writes. Deterministic job IDs make retries idempotent. Rows already in `processing`, `completed`, or `failed` are never selected for publication.

`apps/worker/src/queues/github/github-webhook.worker.ts`, `apps/worker/src/processors/github/github-push.processor.ts`, and `apps/worker/src/runtime.ts` provide the implemented webhook worker lifecycle: Redis readiness, concurrency restricted to 1–32, bounded durable-reference validation, complete execution-time revalidation of nested commit tree IDs, booleans, URLs, identities and paths, execution-time authority revalidation, canonical PostgreSQL activity persistence, monitored fatal run-loop completion, SIGINT/SIGTERM handling, and graceful-then-forced deadline-bounded close. Terminal-failure recording is attempted once and recorder failures are contained; BullMQ stores only the stable `WEBHOOK_PROCESSING_FAILED` reason and never retains raw processor or observability exception text. A fatal run loop marks the process failed and initiates the same idempotent stop path.
...
- `POST /api/v1/reports` with `ReportCreateRequest`, returning `ReportCreateResponse`; accepted work may return `429 RATE_LIMITED`.
- `GET /api/v1/reports` with `ReportListQuery`, returning `ReportListResponse`.
- `GET /api/v1/reports/:id`, returning `ReportDetailResponse`.
- `PUT /api/v1/reports/:id/revision` with `ReportRevisionUpdateRequest`, returning `ReportRevisionUpdateResponse`; accepted work may return `429 RATE_LIMITED`.
- `POST /api/v1/reports/:id/regenerate` with `ReportRegenerationRequest`, returning `ReportRegenerationResponse`; accepted work may return `429 RATE_LIMITED`.
...
Day 8 create behavior requires the authenticated session's `X-CSRF-Token` and aggregates only the authenticated user's currently authorized, tracked GitHub commit activity inside the requested half-open local calendar day. Aggregation is bounded to 10,000 accepted activity rows and 500,000 UTF-8 bytes of evidence messages; larger days fail safely instead of creating an unbounded snapshot. The persisted snapshot contains deterministic global, repository, contributor, and evidence facts and is not recomputed by list/detail reads. One report is allowed per user and report date; duplicates return `409 REPORT_ALREADY_EXISTS`. Queue publication uses queue `report-generation`, job name `generate-report`, job ID `report-<reportId>`, and payload `{ reportId }`. Pending reports are continuously reconciled on API startup and at a bounded interval, including rows previously marked as published, so a lost Redis job is recreated with the same deterministic ID. Report jobs are retained while Day 8 reports remain pending, making the deterministic ID a lifetime deduplication fence; a later report worker may remove a job only after atomically transitioning the report out of `pending`.

List ordering is stable descending `(createdAt, id)`. Its opaque signed cursor is bound to the authenticated user, status filter, and limit; malformed, tampered, or filter-reused cursors return `400 VALIDATION_ERROR`. Detail lookup is owner-scoped and returns `404 REPORT_NOT_FOUND` for both absent and foreign reports.
...
Day 10 revision updates and regeneration are asynchronous render requests. A successful mutation atomically records an authoritative current revision, increments a monotonic render generation, returns the report in `processing`, hides historical artifacts, and durably records the render obligation before queue publication. `PUT .../revision` creates one immutable `manual` revision by applying only the validated sparse prose patch. `POST .../regenerate` preserves the current immutable revision and content and requests a fresh artifact generation; it does not call the narrative provider or overwrite manual edits. Terminal render failure remains `failed` under duplicate or redelivered jobs; only an authenticated, CSRF-protected owner mutation may move it back to `processing` with a new render generation. The API startup/interval reconciler republishes both initial generation and outstanding render obligations using separate bounded batches and database-backed attempt clocks advanced before Redis I/O, so a failing oldest cohort rotates behind untouched work instead of starving later obligations. Jobs retain the deterministic report ID, and a failed or interrupted publication attempt leaves the PostgreSQL obligation outstanding for a later cycle.

The worker renders only the frozen structured schema through a fixed, safely escaped template. It compiles in the pinned `infrastructure/latex` image with no network, no shell escape, a read-only root, dropped capabilities, non-root UID/GID, bounded CPU/memory/PIDs/time, and size-bounded `tmpfs` working directories. Only the read-only generated source and isolated final-output directory are mounted. A database-clock lease plus current-revision and render-generation fence rejects stale output. Immutable generation/attempt-scoped `.tex` and PDF writes occur outside database transactions under one abort deadline shorter than the render lease. A failure aborts its sibling and all writes settle before processing ownership is released. Filesystem mutation runs in killable child-process boundaries, so open, write, sync, permission, link, collision-check, cleanup, and close phases cannot trap the report worker past cancellation. A final fenced transaction activates only the still-current generation's artifact rows, checksums, current paths, completion time, and obligation clearing. Late or stale staged objects are unreachable and cannot become current.

Downloads authorize through `(session user -> report -> current revision -> artifact ID)`, never through a client storage key. The storage adapter enforces server-generated owner/report/revision/generation/attempt keys (while retaining reads of legacy revision-only keys), traversal and symlink rejection, bounded reads, abortable writes, atomic idempotent objects, and immutable conflicts. Before streaming, the API verifies the stored byte length and SHA-256 checksum against database metadata. Missing/foreign artifacts return `404 REPORT_ARTIFACT_NOT_FOUND`; inaccessible or corrupt owned storage returns `503 REPORT_GENERATION_UNAVAILABLE` without exposing backend details.
