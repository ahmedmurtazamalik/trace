-- Preserve workspace report inputs and terminal occurrence/analysis evidence at the database boundary.

CREATE FUNCTION "protect_workspace_report_input_snapshot"()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF OLD."workspace_id" IS NOT NULL
     AND NEW."input_snapshot" IS DISTINCT FROM OLD."input_snapshot" THEN
    RAISE EXCEPTION 'workspace report input snapshot is immutable'
      USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER "reports_workspace_input_snapshot_immutability"
BEFORE UPDATE ON "reports"
FOR EACH ROW
EXECUTE FUNCTION "protect_workspace_report_input_snapshot"();
...
BEGIN
  ...
  IF NEW."status" = 'COMPLETED' AND (NEW."report_id" IS NULL OR NEW."started_at" IS NULL OR NEW."completed_at" IS NULL OR NEW."error" IS NOT NULL) THEN
    RAISE EXCEPTION 'completed workspace report occurrence identity is invalid'
      USING ERRCODE = '23514';
  END IF;
  IF NEW."status" = 'FAILED' AND (NEW."started_at" IS NULL OR NEW."error" IS NULL OR NEW."completed_at" IS NOT NULL) THEN
...
CREATE OR REPLACE FUNCTION "protect_completed_workspace_analysis_run"()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD."status" NOT IN ('COMPLETED', 'FAILED', 'BLOCKED_ACCESS') THEN
      RETURN OLD;
    END IF;
    IF pg_trigger_depth() > 1
       AND NOT EXISTS (SELECT 1 FROM "workspaces" WHERE "id" = OLD."workspace_id") THEN
      RETURN OLD;
    END IF;
    RAISE EXCEPTION 'terminal workspace analysis evidence cannot be deleted'
      USING ERRCODE = '23514';
  END IF;

  IF ROW(NEW."id", NEW."analysis_id", NEW."workspace_id", NEW."repository_id", NEW."created_at")
     IS DISTINCT FROM
     ROW(OLD."id", OLD."analysis_id", OLD."workspace_id", OLD."repository_id", OLD."created_at") THEN
    RAISE EXCEPTION 'workspace analysis run identity is immutable'
      USING ERRCODE = '23514';
  END IF;

  IF OLD."status" IN ('COMPLETED', 'FAILED', 'BLOCKED_ACCESS') AND NEW IS DISTINCT FROM OLD THEN
    RAISE EXCEPTION 'terminal workspace analysis evidence is immutable'
      USING ERRCODE = '23514';
  END IF;

  IF NEW."status" IS DISTINCT FROM OLD."status" AND NOT (
    (OLD."status" = 'PENDING' AND NEW."status" IN ('PROCESSING', 'COMPLETED', 'FAILED', 'BLOCKED_ACCESS')) OR
    (OLD."status" = 'PROCESSING' AND NEW."status" IN ('PENDING', 'COMPLETED', 'FAILED', 'BLOCKED_ACCESS'))
  ) THEN
    RAISE EXCEPTION 'workspace analysis lifecycle transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  IF NEW."status" = 'COMPLETED' AND (NEW."to_sha" IS NULL OR NEW."started_at" IS NULL OR NEW."completed_at" IS NULL OR NEW."error" IS NOT NULL) THEN
    RAISE EXCEPTION 'completed workspace analysis identity is invalid'
      USING ERRCODE = '23514';
  END IF;
  IF NEW."status" IN ('FAILED', 'BLOCKED_ACCESS') AND (NEW."completed_at" IS NULL OR NEW."error" IS NULL) THEN
    RAISE EXCEPTION 'terminal workspace analysis identity is invalid'
      USING ERRCODE = '23514';
  END IF;
